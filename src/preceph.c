...
static int addpeph(nav_t *nav, peph_t *peph)
{
    ...
    if (nav->ne>=nav->nemax) {
        ...
        if (!(nav_peph=(peph_t *)realloc(nav->peph,sizeof(peph_t)*nav->nemax))) {
            RTKtrace(1,"readsp3b malloc error n=%d\n",nav->nemax);
                     ...
                     char *tsys, int index, int opt, nav_t *nav)
{
    ...
    RTKtrace(3,"readsp3b: type=%c ns=%d index=%d opt=%d\n",type,ns,index,opt);
    ...
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        if (buff[0]!='*'||str2time(buff,3,28,&time)) {
            RTKtrace(2,"sp3 invalid epoch %31.31s\n",buff);
            continue;
        }
...
extern void readsp3(const char *file, nav_t *nav, int opt)
{
    ...
    for (i=j=0;i<n;i++) {
        ...
        if (!(fp=fopen(efiles[i],"r"))) {
            RTKtrace(2,"sp3 file open error %s\n",efiles[i]);
            continue;
        }
        /* read sp3 header */
...
extern int readsap(const char *file, gtime_t time, nav_t *nav)
{
    ...
    RTKtrace(3,"readsap : file=%s time=%s\n",file,time_str(time,0));
...
static int readdcbf(const char *file, nav_t *nav)
{
    ...
    RTKtrace(3,"readdcbf: file=%s\n",file);
    
    if (!(fp=fopen(file,"r"))) {
        RTKtrace(2,"dcb parameters file open error: %s\n",file);
        return 0;
    }
    while (fgets(buff,sizeof(buff),fp)) {
                   ...
                   double *dts, double *vare, double *varc)
{
    ...
    RTKtrace(4,"pephpos : time=%s sat=%2d\n",time_str(time,3),sat);
    ...
    if (nav->ne<NMAX+1||
        ...
        RTKtrace(2,"no prec ephem %s sat=%2d\n",time_str(time,0),sat);
        return 0;
    }
    /* binary search */
    ...
    for (j=0;j<=NMAX;j++) {
        ...
        if (norm(nav->peph[i+j].pos[sat-1],3)<=0.0) {
            RTKtrace(2,"prec ephem outage %s sat=%2d\n",time_str(time,0),sat);
            return 0;
        }
    }
    for (j=0;j<=NMAX;j++) {
                   ...
                   double *varc)
{
    ...
    if (t[0]<=0.0) {
        ...
        RTKtrace(3,"prec clock outage %s sat=%2d\n",time_str(time,0),sat);
        return 0;
    }
    if (varc) *varc=SQR(std);
    return 1;
                      ...
                      double *dant)
{
    ...
    RTKtrace(4,"satantoff: time=%s sat=%2d\n",time_str(time,3),sat);
                    ...
                    double *rs, double *dts, double *var)
{
    ...
    RTKtrace(4,"peph2pos: time=%s sat=%2d opt=%d\n",time_str(time,3),sat,opt);
