                     ...
                     double rb, nav_t *nav)
{
    ...
    if (nav->nt>=nav->ntmax) {
        ...
        if (!(nav_tec=(tec_t *)realloc(nav->tec,sizeof(tec_t)*nav->ntmax))) {
            RTKtrace(1,"readionex malloc error ntmax=%d\n",nav->ntmax);
...
static void readionexdcb(FILE *fp, double *dcb, double *rms)
{
    ...
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        if (strstr(label,"PRN / BIAS / RMS")==label) {
            ...
            if (!(sat=satid2no(id))) {
                RTKtrace(2,"ionex invalid satellite: %s\n",id);
                continue;
            }
    ...
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        if (strstr(label,"START OF TEC MAP")==label) {
            ...
            if (str2time(buff,0,36,&time)) {
                RTKtrace(2,"ionex epoch invalid: %-36.36s\n",buff);
                continue;
            }
            if (type==2) {
...
extern void readtec(const char *file, nav_t *nav, int opt)
{
    ...
    for (i=0;i<n;i++) {
        if (!(fp=fopen(efiles[i],"r"))) {
            RTKtrace(2,"ionex file open error %s\n",efiles[i]);
            continue;
        }
        /* read ionex header */
        if (readionexh(fp,lats,lons,hgts,&rb,&nexp,dcb,rms)<=0.0) {
            RTKtrace(2,"ionex file format error %s\n",efiles[i]);
            continue;
        }
        /* read ionex body */
                     ...
                     double *rms)
{
    ...
    RTKtrace(3,"interptec: k=%d posp=%.2f %.2f\n",k,posp[0]*R2D,posp[1]*R2D);
    *value=*rms=0.0;
                    ...
                    const double *azel, int opt, double *delay, double *var)
{
    ...
    RTKtrace(3,"iondelay: time=%s pos=%.1f %.1f azel=%.1f %.1f\n",time_str(time,0),
    ...
    *delay=*var=0.0;
    ...
    RTKtrace(4,"iondelay: delay=%7.2f std=%6.2f\n",*delay,sqrt(*var));
                  ...
                  const double *azel, int opt, double *delay, double *var)
{
    ...
    RTKtrace(3,"iontec  : time=%s pos=%.1f %.1f azel=%.1f %.1f\n",time_str(time,0),
          pos[0]*R2D,pos[1]*R2D,azel[0]*R2D,azel[1]*R2D);
    ...
    if (i==0||i>=nav->nt) {
        RTKtrace(2,"%s: tec grid out of period\n",time_str(time,0));
        return 0;
    }
    ...
    RTKtrace(3,"iontec  : delay=%5.2f std=%5.2f\n",*delay,sqrt(*var));
    return 1;
}
