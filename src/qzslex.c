                         ...
                         gtime_t *toe)
{
    ...
    *toe=gpst2time(week,toes);
    ...
    RTKtrace(3,"decode_lextof: tow=%d week=%d toe=%d\n",tow,week,toes);
    ...
    time2str(*tof,s1,3);
    time2str(*toe,s2,3);
    RTKtrace(4,"decode_lextof: tof=%s toe=%s\n",s1,s2);
    return i;
}
                            ...
                            nav_t *nav)
{
    ...
    RTKtrace(3,"decode_lexhealth: tof=%s\n",time_str(tof,0));
                         ...
                         nav_t *nav)
{
    ...
    RTKtrace(3,"decode_lexeph: toe=%s\n",time_str(toe,0));
    ...
    if      (  1<=prn&&prn<= 32) sat=satno(SYS_GPS,prn);
        ...
        RTKtrace(2,"lex ephemeris prn error prn=%d\n",prn);
        return i;
    }
    eph.toe=toe;
    eph.sat=sat;
    ...
    RTKtrace(4,"sat=%2d toe=%s pos=%.3f %.3f %.3f vel=%.5f %.5f %.5f\n",
          sat,time_str(toe,0),eph.pos[0],eph.pos[1],eph.pos[2],
          eph.vel[0],eph.vel[1],eph.vel[2]);
    RTKtrace(4,"clk=%11.3f %8.5f tgd=%7.3f\n",eph.af0*1E9,eph.af1*1E9,
          eph.tgd*1E9);
    RTKtrace(4,"isc=%6.3f %6.3f %6.3f %6.3f %6.3f %6.3f %6.3f\n",
                         ...
                         nav_t *nav)
{
    ...
    RTKtrace(3,"decode_lexion: tof=%s\n",time_str(tof,0));
    ...
    RTKtrace(4,"t0=%s tspan=%.0f pos0=%.1f %.1f coef=%.3f %.3f %.3f %.3f %.3f %.3f\n",
...
static int lex2rtcm(const unsigned char *msg, int i, unsigned char *buff)
{
    ...
    switch ((type=getbitu(msg,i,12))) {
        ...
        default:
            if (type) RTKtrace(2,"lex 12: unsupported type=%4d\n",type);
            return 0;
    }
    n=(n+7)/8; /* message length (bytes) */
    
    if (i+n*8>LEXFRMLEN-LEXRSLEN) {
        RTKtrace(2,"lex 12: invalid ssr size: len=%4d\n",n);
        return 0;
    }
...
{
    RTKtrace(3,"lexupdatecorr: type=%d\n",msg->type);
    ...
    RTKtrace(2,"unsupported lex message: type=%2d\n",msg->type);
    return 0;
}
...
extern int lexreadmsg(const char *file, int sel, lex_t *lex)
{
    ...
    RTKtrace(3,"readmsgs: file=%s sel=%d\n",file,sel);
    
    if (!(p=strrchr(file,'.'))||(strcmp(p,".lex")&&strcmp(p,".LEX"))) return 0;
    
    if (!(fp=fopen(file,"r"))) {
        RTKtrace(2,"lex message log open error: %s\n",file);
        return 0;
    }
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        if (lex->n>=lex->nmax) {
            ...
            if (!(lex_msgs=(lexmsg_t *)realloc(lex->msgs,lex->nmax*sizeof(lexmsg_t)))) {
                RTKtrace(1,"lexreadmsg malloc error: nmax=%d\n",lex->nmax);
                      ...
                      const char *outfile)
{
    ...
    RTKtrace(3,"lexconvbin:type=%d infile=%s outfile=%s\n",type,infile,outfile);
    
    if (!(ifp=fopen(infile,"rb"))) {
        RTKtrace(1,"lexconvbin infile open error: %s\n",infile);
        return 0;
    }
    if (!(ofp=fopen(outfile,"w"))) {
        RTKtrace(1,"lexconvbin outfile open error: %s\n",outfile);
        fclose(ifp);
        return 0;
    }
    while (fread(buff,1,len,ifp)==len) {
        ...
        if (preamb!=LEXFRMPREAMB) {
            RTKtrace(1,"lex frame preamble error: preamb=%08X\n",preamb);
            continue;
        }
        for (j=0;j<212;j++) {
                      ...
                      double *dts, double *var)
{
    ...
    RTKtrace(3,"lexsatpos: time=%s sat=%2d\n",time_str(time,3),sat);
    ...
    if (eph->sat!=sat||eph->toe.time==0) {
         RTKtrace(2,"no lex ephemeris: time=%s sat=%2d\n",time_str(time,0),sat);
         return 0;
    }
    if (fabs(t=timediff(time,eph->toe))>LEXEPHMAXAGE) {
         RTKtrace(2,"lex ephemeris age error: time=%s sat=%2d t=%.3f\n",
               time_str(time,0),sat,t);
         return 0;
    }
#if 0
    if (eph->health&0x18) {
         RTKtrace(2,"lex ephemeris unhealthy: sat=%2d health=0x%02X\n",sat,eph->health);
         return 0;
    }
#endif
                      ...
                      const double *azel, double *delay, double *var)
{
    ...
    RTKtrace(4,"lexioncorr: time=%s pos=%.3f %.3f azel=%.3f %.3f\n",time_str(time,3),
    ...
    *delay=*var=0.0;
    ...
    if (fabs(tt)>nav->lexion.tspan) {
        RTKtrace(2,"lex iono age error: tt=%.0f tspan=%.0f\n",tt,nav->lexion.tspan);
        return 0;
    }
    ...
    if (pos[0]> 45.5*D2R||pos[0]< 26.0*D2R||
        ...
        RTKtrace(2,"lex iono out of coverage pos=%.3f %.3f\n",pos[0]*R2D,pos[1]*R2D);
        return 0;
    }
#endif
    ...
    RTKtrace(4,"lexioncorr: pppos=%.3f %.3f\n",latpp*R2D,lonpp*R2D);
    ...
    RTKtrace(4,"lexioncorr: pos0=%.1f %.1f dlat=%.1f dlon=%.1f\n",
    ...
    for (n=0;n<=2;n++) for (m=0;m<=1;m++) {
        ...
        *delay+=F*Enm*pow(dlat,n)*pow(dlon,m);
        ...
        RTKtrace(5,"lexioncorr: F=%8.3f Enm[%d][%d]=%8.3f delay=%8.3f\n",F,n,m,Enm,
              F*Enm*pow(dlat,n)*pow(dlon,m));
    }
    RTKtrace(4,"lexioncorr: time=%s delay=%.3f\n",time_str(time,0),*delay);
