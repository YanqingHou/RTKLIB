...
static int savefloatsols(rtk_t *rtk, double *bias, double *xa)
{
   ...
   /* for (i=0;i<nbsl;i++) for (j=0;j<nbsl;j++) Qbsl[i+j*nbsl]=Qy[   i+j*ny]; */
    RTKtrace(1,s1);
	RTKtrace(1,"aflt\n"); tracemat(1,y+na,1,nb,10,5); /*float ambiguities*/
	RTKtrace(1,"bflt\n"); tracemat(1,bsl,1,nbsl,10,5);    /* float baseline solution 	*/
	RTKtrace(1,"Qah\n");  tracemat(1,Qb,nb,nb,10,5);  /* Qahat_ahat */
	RTKtrace(1,"Qbh\n");  tracemat(1,Qbsl,nbsl,nbsl,10,5);  /* Qahat_ahat */
    RTKtrace(1,"Qba\n");  tracemat(1,Qab,nbsl,nb,10,5);  /* Qbhat_ahat */
...
static int resamb_LAMBDA(rtk_t *rtk, double *bias, double *xa) {
	...
	else if (opt->thresar[0] == 101) {
		Pf = 0.01;
	}
	else if (opt->thresar[0] == 1000) {
		Pf = 0.001;
	}
	else if (opt->thresar[0] == 1001) {
		Pf = 0.001;
	}
	else {
		Pf = 0.01;
