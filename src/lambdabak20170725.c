...
static double interp1q(double *x, double *y, int dim, double xi)
{
 int lowidx=0, medidx, highidx=dim-1;
 static double t=0;
 if (xi<x[lowidx]||xi>x[highidx]) {exit(0);
 }
 else if(xi==x[lowidx]) {
...
static double invratio(double Pf, double PfILS, int n)
{
static double table1[1984]={0, 0.0010, 0.0012, 0.0015, 0.0020, 0.0050, 0.0100, 0.0150, 0.0200, 0.0250, 0.0300, 0.0350, 0.0400, 0.0450, 0.0500, 0.0550, 0.0600, 0.0650, 0.0700, 0.0750, 0.0800, 0.0850, 0.0900, 0.0950, 0.1000, 0.1200, 0.1500, 0.2000, 0.2500, 0.2600, 1,
...
static double table10[1271]={0, 0.0100, 0.0110, 0.0160, 0.0210, 0.0260, 0.0310, 0.0360, 0.0410, 0.0460, 0.0510, 0.0560, 0.0610, 0.0660, 0.0710, 0.0760, 0.0810, 0.0860, 0.0910, 0.0960, 0.1200, 0.1500, 0.2000, 0.2500, 0.3000, 0.3500, 0.4000, 0.4500, 0.4990, 0.5000, 1,
...
output: P(x<X)
*/
static double cnd_manual(double x)
{
  double L, K, w ;
  /* constants */
...
static double successrate(int k, int nb, double *D)
{
  int i;
  double t, Ps=1.0;
  for (i = k; i<nb; i++) {
...
static double cpmufit(double Pftol,double PfILS, int n)
{                     /*Pf_tol=0.01
n	q1	      q2	q3 */

	  static double coeff01[198]={
	0.0916,	-0.5801,	-0.285,
...
/* WL/NL integer least-square estimation ------------------------------
* integer least-square estimation. reduction is performed by lambda (ref.[1]),
* and search by mlambda (ref.[2]).
* args   : int    n      I  number of float parameters
*          int    m      I  number of fixed solutions
...
*-----------------------------------------------------------------------------*/
/*
extern int wlnlar(int *nnbs, double *wlfreqs, double *wlams, int sysn,
int m, const double *a, const double *Q, double *F, double *s, double *mu, double Pf)
{
	int info,i,j,k,n=0,np;
	double *L,*D,*Z,*z,*E,*Tr,*ap,*Qp,*DP;
	double ps=0;
	for (i = 0; i < sysn; i++) {
		n+=nnbs[i]-1;
	}
	if (n<=0||m<=0) return -1;
	L=zeros(n,n); D=mat(n,1); Z=eye(n); z=mat(n,1),E=mat(n,m);
	ap=mat(n,1); Qp=mat(n,n); DP=mat(n,n);
	//Change the order of ambiguities: from [Ng_1,Ng_w,Nb_1,Nb_w] to [Ng_1,Nb_1,Ng_w,Nb_w]
	Tr=zeros(n,n);
	switch (sysn) {
		case 1: //only one system
		Tr=eye(n);
		break;
		case 2: //two systems Tr=[eye(ng),0,      0,        0;
				 //				 0        0,      eye(nb),  0;
				  //				 0,       eye(ng),0,        0;
				  //				 0,       0,      0,  eye(nb)]
		for (i = 0; i < nnbs[0]-1; i++) {
			Tr[i*n+i]=1;
		}
		for (i=nnbs[0]-1; i< 2*nnbs[0]-2; i++) {  //eye(ng)
			Tr[i*n+i+nnbs[1]-1]=1;
		}
		for (i = 2*nnbs[0]-2; i < 2*nnbs[0]-2+nnbs[1]-1; i++) {//eye(nb)
			Tr[i*n+i-nnbs[0]+1]=1;
		}
		for (i=2*nnbs[0]-2+nnbs[1]-1; i< 2*nnbs[0]-2+2*nnbs[1]-2; i++) {
			Tr[i*n+i]=1;
		}

		break;
		case 3: //three systems
		break;
	default:
		;
	}
	matmul("NN",n,1,n,1.0,Tr,a,0.0,ap);//ap=Tr*a; Qp=Tr*Q*Tr'
	matmul("NN",n,n,n,1.0,Tr,Q,0.0,DP);
	matmul("NT",n,n,n,1.0,DP,Tr,0.0,Qp);
	//round to WL ambiguities
	np=n/2;
	for (i = 0, k=0; i < sysn; k+=2*nnbs[i]-2, i++) {
		 //round the wl ambiguity for system i
		 for (j = k+nnbs[i]-1; j<k+2*nnbs[i]-2; j++) {
			  z[j]=ROUND(a[j]/wlams[sysn*2+1]);
		 }
		 //update the L1 ambiguity

		 //resolve L1 ambiguity using lambda
	}
	// LD factorization
	if (!(info=LD(n,Q,L,D))) {
