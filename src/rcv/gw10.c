...
static int decode_gw10raw(raw_t *raw)
{
    ...
    for (i=n=0,p+=8;i<16&&n<MAXOBS;i++,p+=23) {
        ...
        if (!(sat=satno(prn<=MAXPRNGPS?SYS_GPS:SYS_SBS,prn))) {
            RTKtrace(2,"gw10raw satellite number error: prn=%d\n",prn);
            continue;
        }
        pr =R8(p+ 2)-toff;
        snr=U2(p+16);
        ...
        flg=U1(p+22);
        if (flg&0x3) {
            RTKtrace(2,"gw10raw raw data invalid: prn=%d\n",prn);
            continue;
        }
...
static int decode_gw10gps(raw_t *raw)
{
    ...
    RTKtrace(4,"decode_gw10gps: len=%d\n",raw->len);
    
    tow=U4(p)/1000.0; p+=4;
    prn=U1(p);        p+=1;
    if (!(sat=satno(SYS_GPS,prn))) {
        RTKtrace(2,"gw10 gps satellite number error: tow=%.1f prn=%d\n",tow,prn);
        return -1;
    }
    for (i=0;i<10;i++) {
        ...
        if (!check_parity(buff,subfrm+i*3)) {
            RTKtrace(2,"gw10 gps frame parity error: tow=%.1f prn=%2d word=%2d\n",
                 tow,prn,i+1);
            return -1;
        }
    }
    ...
    if (id<1||5<id) {
        RTKtrace(2,"gw10 gps frame id error: tow=%.1f prn=%2d id=%d\n",tow,prn,id);
        return -1;
    }
...
static int decode_gw10sbs(raw_t *raw)
{
    ...
    RTKtrace(4,"decode_gw10sbs : len=%d\n",raw->len);
    
    tow=U4(p)/1000.0;
    prn=U1(p+4);
    if (prn<MINPRNSBS||MAXPRNSBS<prn) {
        RTKtrace(2,"gw10 sbs satellite number error: prn=%d\n",prn);
        return -1;
    }
    raw->sbsmsg.prn=prn;
...
static int decode_gw10reph(raw_t *raw)
{
    ...
    RTKtrace(4,"decode_gw10reph: len=%d\n",raw->len);
    
    prn=U1(p);
    if (!(sat=satno(SYS_GPS,prn))) {
        RTKtrace(2,"gw10 raw ephemeris satellite number error: prn=%d\n",prn);
        return -1;
    }
    for (i=0;i<90;i++) {
        buff[i]=*(p+1+i);
    ...
    if (decode_frame(buff    ,&eph,NULL,NULL,NULL,NULL)!=1||
        ...
        RTKtrace(2,"gw10 raw ephemeris navigation frame error: prn=%d\n",prn);
        return -1;
    }
...
static int decode_gw10sol(raw_t *raw)
{
    ...
    RTKtrace(4,"decode_gw10sol : len=%d\n",raw->len);
    
    if (U2(p+42)&0xC00) { /* time valid? */
        RTKtrace(2,"gw10 sol time/day invalid\n");
        return 0;
    }
    sec=U4(p+27)/16384.0;
...
static int decode_gw10(raw_t *raw)
{
    ...
    RTKtrace(3,"decode_gw10: type=0x%02X len=%d\n",type,raw->len);
...
{
    int stat;
    RTKtrace(5,"input_gw10: data=%02x\n",data);
