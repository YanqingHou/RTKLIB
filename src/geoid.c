...
static double geoidh_emb(const double *pos)
{
    ...
    if (pos[1]<range[0]||range[1]<pos[1]||pos[0]<range[2]||range[3]<pos[0]) {
        RTKtrace(2,"out of geoid model range: lat=%.3f lon=%.3f\n",pos[0],pos[1]);
        return 0.0;
    }
...
static short fget2b(FILE *fp, long off)
{
    ...
    if (fseek(fp,off,SEEK_SET)==EOF||fread(v,2,1,fp)<1) {
        RTKtrace(2,"geoid data file range error: off=%ld\n",off);
    }
...
static double fgetgsi(FILE *fp, int nlon, int nlat, int i, int j)
{
    ...
    if (fseek(fp,off,SEEK_SET)==EOF||fread(buff,wf,1,fp)<1) {
        RTKtrace(2,"out of range for gsi geoid: i=%d j=%d\n",i,j);
        return 0.0;
    }
    if (sscanf(buff,"%lf",&v)<1) {
        RTKtrace(2,"gsi geoid data format error: i=%d j=%d buff=%s\n",i,j,buff);
        return 0.0;
    }
    return v;
}
...
static double geoidh_gsi(const double *pos)
{
    ...
    if (!fp_geoid||pos[1]<lon0||lon1<pos[1]||pos[0]<lat0||lat1<pos[0]) {
        RTKtrace(2,"out of range for gsi geoid: lat=%.3f lon=%.3f\n",pos[0],pos[1]);
        return 0.0;
    }
    a=(pos[1]-lon0)/dlon;
    b=(pos[0]-lat0)/dlat;
    ...
    if (y[0]==999.0||y[1]==999.0||y[2]==999.0||y[3]==999.0) {
        RTKtrace(2,"geoidh_gsi: data outage (lat=%.3f lon=%.3f)\n",pos[0],pos[1]);
        return 0.0;
    }
    return interpb(y,a,b);
}
...
{
    RTKtrace(3,"opengeoid: model=%d file=%s\n",model,file);
    ...
    if (model!=GEOID_EGM96_M150 &&model!=GEOID_EGM2008_M25&&
        ...
        RTKtrace(2,"invalid geoid model: model=%d file=%s\n",model,file);
        return 0;
    }
    if (!(fp_geoid=fopen(file,"rb"))) {
        RTKtrace(2,"geoid model file open error: model=%d file=%s\n",model,file);
        return 0;
    }
    model_geoid=model;
    return 1;
...
extern double geoidh(const double *pos)
{
    ...
    if (posd[1]<0.0||360.0-1E-12<posd[1]||posd[0]<-90.0||90.0<posd[0]) {
        RTKtrace(2,"out of range for geoid model: lat=%.3f lon=%.3f\n",posd[0],posd[1]);
        return 0.0;
    }
    switch (model_geoid) {
    ...
    if (fabs(h)>200.0) {
        RTKtrace(2,"invalid geoid model: lat=%.3f lon=%.3f h=%.3f\n",posd[0],posd[1],h);
        return 0.0;
    }
    return h;
}
