...
static int decode_line2(const char *buff, tled_t *data)
{
    ...
    if (strcmp(satno,data->satno)) {
        RTKtrace(2,"tle satno mismatch: %s %s\n",data->satno,satno);
        return 0;
    }
    if (data->n<=0.0||data->ecc<0.0) {
        RTKtrace(2,"tle data error: %s\n",satno);
        return 0;
    }
    return 1;
}
...
extern int tle_read(const char *file, tle_t *tle)
{
    ...
    if (!(fp=fopen(file,"r"))) {
        RTKtrace(2,"tle file open error: %s\n",file);
        return 0;
    }
...
extern int tle_name_read(const char *file, tle_t *tle)
{
    ...
    if (!(fp=fopen(file,"r"))) {
        RTKtrace(2,"tle satellite name file open error: %s\n",file);
        return 0;
    }
    while (fgets(buff,sizeof(buff),fp)) {
        ...
        if (i>=tle->n) {
            RTKtrace(3,"no tle data: satno=%s desig=%s\n",satno,desig);
            continue;
        }
                   ...
                   double *rs)
{
    ...
    if (stat) {
        RTKtrace(3,"no tle data: name=%s satno=%s desig=%s\n",name,satno,desig);
        return 0;
    }
    tutc=gpst2utc(time);
