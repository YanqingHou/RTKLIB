...
static int addobsdata(obs_t *obs, const obsd_t *data)
{
    ...
    if (obs->nmax<=obs->n) {
        ...
        if (!(obs_data=(obsd_t *)realloc(obs->data,sizeof(obsd_t)*obs->nmax))) {
            RTKtrace(1,"addobsdata: memalloc error n=%dx%d\n",sizeof(obsd_t),obs->nmax);
...
#if 0 /* for debug */
    for (i=0;i<n;i++) {
        RTKtrace(2,"set_index: sys=%2d,tobs=%s code=%2d pri=%2d frq=%d pos=%d shift=%5.2f\n",
                       ...
                       geph_t *geph)
{
    ...
    if (satsys(sat,NULL)!=SYS_GLO) {
        RTKtrace(2,"glonass ephemeris error: invalid satellite sat=%2d\n",sat);
        return 0;
    }
    *geph=geph0;
                       ...
                       seph_t *seph)
{
    ...
    if (satsys(sat,NULL)!=SYS_SBS) {
        RTKtrace(2,"geo ephemeris error: invalid satellite sat=%2d\n",sat);
        return 0;
    }
    *seph=seph0;
